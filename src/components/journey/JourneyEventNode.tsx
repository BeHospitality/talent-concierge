...
export function JourneyEventNode({ event, isCurrentPhase, onCompleted }: JourneyEventNodeProps) {
  ...
  const renderNode = () => {
    ...
    if (isOverdue) {
      return <AlertCircle className="w-4 h-4 text-destructive flex-shrink-0 animate-pulse" />;
    }
    if (isMilestone) {
      return <Diamond className="w-4 h-4 text-primary flex-shrink-0" />;
    }
    if (isDueToday || event.status === "active") {
      return <Circle className="w-4 h-4 text-primary flex-shrink-0 animate-pulse" />;
    }
  ...
  const renderStatus = () => {
    if (isCompleted) {
      return <Badge variant="outline" className="text-[10px] bg-success/10 text-success border-success/20">✅ Complete</Badge>;
    }
    if (isSkipped) {
      return <Badge variant="outline" className="text-[10px] bg-muted/50 text-muted-foreground border-border/30">Skipped</Badge>;
    }
    if (isOverdue) {
      return <Badge variant="outline" className="text-[10px] bg-destructive/10 text-destructive border-destructive/20">🔴 {daysOverdue}d overdue</Badge>;
    }
    if (isDueToday) {
      return <Badge variant="outline" className="text-[10px] bg-warning/10 text-warning border-warning/20">⏳ Due today</Badge>;
    }
    if (scheduledDate) {
      return <Badge variant="outline" className="text-[10px] text-muted-foreground">📅 {format(scheduledDate, "MMM d")}</Badge>;
    }
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {event.completed_at && (
              <span className="text-[10px] text-success">
                Completed {format(new Date(event.completed_at), "MMM d")}
              </span>
            )}
          </div>
          {canMarkComplete && (
            <Button variant="outline" size="sm" className="mt-1.5 h-7 text-xs gap-1" onClick={() => setCompleteOpen(true)}>
              Mark Complete
            </Button>
          )}
        </div>
